...
const TourDetailsImageEdit = () => {
    const [dropdownOpen, setDropdownOpen] = useState(false);
    const [toursDetails, setToursDetails] = useState([]);
    const location = useLocation();
    const tourdetailsimageid = new URLSearchParams(location.search).get("id");

    const [tourDetailsImage, setTourDetailsImage] = useState(null); 
    ...
    const fetchToursDetails = async () => {
        try {
            const response = await fetch('https://localhost:7026/api/admin/tourdetailsimage/edit', {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        const fetchTourDetailsImageData = async () => {
            try {
                const response = await fetch(`https://localhost:7026/api/admin/tourdetailsimage/edit/${tourdetailsimageid}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (response.ok) {
                    const tourdetailsimageData = await response.json();
                    setTourDetailsImage(tourdetailsimageData); 
                }
            } catch (error) {
        ...
        fetchTourDetailsImageData();
    }, [tourdetailsimageid]);

    useEffect(() => {
        if (tourDetailsImage) {
            setFormData({
                name: tourDetailsImage.name,
                url: tourDetailsImage.url,
                tour_details_id: tourDetailsImage.tour_details_id,
            });
        }
    }, [tourDetailsImage]);
                ...
                <div id="layoutSidenav_content">
                    <main>
                        ...
                        <form onSubmit={handleSubmit} className="create-employee-form">
                            <h2 className="form-title">Cập Nhật Hình Ảnh Tour Của Bạn</h2>
...
export default TourDetailsImageEdit;
