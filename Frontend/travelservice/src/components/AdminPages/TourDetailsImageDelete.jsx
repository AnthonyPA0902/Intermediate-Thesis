...
const TourDetailsImageDelete = () => {
    ...
    useEffect(() => {
        fetch(`https://localhost:7026/api/admin/tourdetailsimage/delete/${tourDetailsImageid}`, {
            method: 'GET',
            headers: {
    ...
    const handleDelete = () => {
        ...
        .then((response) => {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Xóa hình ảnh tour thành công',
                showConfirmButton: false,
                timer: 2000 
            }).then(() => {
                window.location.href = "/admin/tourdetailsimage";
            });
        })
        .catch((error) => console.error(error));
    };
    ...
    return (
        ...
        <div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content" style={{margin: 20}}>
                <main>
                    <Link to={`/admin/tourdetailsimage`}><button className="back-btn">Quay trở lại</button></Link>
                    <h1 className="heading">Kiểm Tra Dữ Liệu Trước Khi Xóa:</h1>
                    <div className="info-table">
                        <div className="info-row">
                            <span className="info-label">ID:</span>
                            <span className="info-value">{tourDetailsImage.id}</span>
                        </div>
                        <div className="info-row">
                            <span className="info-label">Tên Hình Ảnh:</span>
                            <span className="info-value">{tourDetailsImage.name}</span>
                        </div>
                        <div className="info-row">
                            <span className="info-label">Đường Dẫn</span>
                            <span className="info-value">{tourDetailsImage.url}</span>
                        </div>
                    </div>
...
export default TourDetailsImageDelete;
